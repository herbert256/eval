...
fun ChessBoardView(
    ...
    val pieceImages by produceState(PieceImages.cached, context.applicationContext) {
        value = PieceImages.load(context)
    }
    ...
    fun positionToSquare(x: Float, y: Float, size: Float): Square? {
        if (!size.isFinite() || size <= 0 || !x.isFinite() || !y.isFinite() ||
            x < 0 || y < 0 || x >= size * 8 || y >= size * 8) return null
        val file = (x / size).toInt()
        val rank = (y / size).toInt()
...
/**
 * Draws the board squares with colors, last move highlight, selection highlight,
 * and legal move indicators (dots for empty squares, rings for captures).
 */
...
/**
 * Draws chess pieces on the board, skipping any piece currently being dragged.
 * Applies custom color tinting when non-default piece colors are configured.
 */
private fun DrawScope.drawPieces(
    ...
    board: ChessBoard,
    dragFromSquare: Square?,
    pieceImages: Map<Pair<PieceColor, PieceType>, ImageBitmap?>,
    ...
    for (rank in 0..7) {
        for (file in 0..7) {
            ...
            // Skip if piece is being dragged
            if (dragFromSquare == square) continue

            val piece = board.getPiece(displayFile, displayRank) ?: continue
...
/**
 * Draws move arrows on the board (up to 8 from PV line or multi-lines).
 * Arrows are drawn in reverse index order so the first arrow (thickest) appears on top.
 * Includes text labels: move numbers for main line mode, scores for multi-lines mode.
 */
...
/**
 * Draws the piece currently being dragged, rendered slightly larger and centered on the drag position.
 * Applies the same color tinting logic as board pieces.
 */
private fun DrawScope.drawDraggedPiece(
    squareSize: Float,
    dragFromSquare: Square?,
    dragPosition: Offset?,
    board: ChessBoard,
    ...
    val fromSquare = dragFromSquare ?: return
    val pos = dragPosition ?: return
    val piece = board.getPiece(fromSquare) ?: return
...
/**
 * Draws file labels (a-h) at the bottom and rank labels (1-8) at the left of the board.
 */
